...
export default function HeroSection({ meta, onLiveClick }: Props) {
  useEffect(() => {
    if (document.querySelector('script[data-hero-canvas]')) return
    const script = document.createElement('script')
    script.src = './assets/home-hero-canvas.js'
    script.setAttribute('data-hero-canvas', '')
  ...
  return (
    <section id="hero">
      ...
      <div id="hb" style={{ flex: 1, height: 'auto', paddingTop: 150, paddingBottom: 150, fontWeight: 800, margin: '6px 0 0' }}>
        <div id="lw">
          <FKLogo />
        </div>
        <p className="kicker"></p>
        <h1 className="ht1" style={{ fontSize: 60, fontWeight: 700 }}>{meta.heroTitle}</h1>
        <h1 className="ht2" style={{ fontSize: 60 }}></h1>
        <p className="hsub" id="hero-sub" style={{ fontFamily: 'Pretendard', fontWeight: 500, fontSize: 24, color: 'rgb(226,238,255)', margin: '0 0 40px', lineHeight: 1, letterSpacing: '.5px' }}>
        ...
        <div className="ctas" style={{ flexDirection: 'column', margin: '40px 0 0' }}>
          <a
            ...
            style={{ width: 402, justifyContent: 'center', padding: '16px 24px', height: 'auto', fontSize: 16, background: '#fa2e43', boxShadow: 'none' }}
          >
            <span>{meta.ctaText}</span> <span className="arrow">→</span>
          </a>
          <span className="chint" id="hero-hint" style={{ fontFamily: 'Pretendard', fontWeight: 400, fontSize: 14, letterSpacing: '-.5px', color: 'rgba(255,255,255,0.7)', textAlign: 'center', lineHeight: 1.6 }}>
            {meta.liveHintPrefix}
            <strong style={{ color: 'rgba(160,200,240,.75)' }}>{meta.liveHintSession}</strong>
            {meta.liveHintSuffix}
          </span>
        </div>
      </div>
      ...
      <div id="meta" style={{ padding: '36px 60px' }}>
        {meta.metaItems.map((item) => (
          <div key={item.key} className="mi">
            <span className="mk" style={{ fontFamily: 'Pretendard', fontWeight: 500, fontSize: 12 }}>{item.key}</span>
            <span className="mv" style={{ fontSize: 16 }}>
              {item.value}
              {item.sub && <span className="sub" style={{ fontSize: 16 }}> {item.sub}</span>}
            </span>
          </div>
        ))}
      </div>
