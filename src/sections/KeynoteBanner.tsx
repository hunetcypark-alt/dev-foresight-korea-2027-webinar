...
interface Props { keynote: KeynoteData }

export default function KeynoteBanner({ keynote }: Props) {
  return (
    <>
      ...
      <div className="spb spb-keynote reveal" id="kn-banner" style={{ '--acc': '#5a8dff' } as React.CSSProperties}>
        ...
        <div className="spb-photo">
          <img src={keynote.photo} alt="키노트 연사" />
        </div>
        <div className="spb-inner">
          <div className="eyebrow spb-kn-eyebrow"><span>KEYNOTE</span></div>
          <h3 className="spb-title">
            AX: Transforming into <br className="br-mob" />an<br className="br-pc" /> AI-Native Company
          </h3>
          <div className="spb-meta">
            <div className="spb-company">{keynote.company}</div>
            <div className="spb-kn-name-row">
              <div className="spb-name">{keynote.name.split(' ')[0]} <span>{keynote.name.split(' ').slice(1).join(' ')}</span></div>
